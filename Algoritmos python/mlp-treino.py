...
class Iris(Dataset):
    def __init__(
        ...
        if treino:
            self.x = torch.tensor(
                np.loadtxt(self.dir / 'x_treino.csv', delimiter=',')
            )
            self.y = torch.tensor(
                np.loadtxt(self.dir / 'y_treino.csv', delimiter=',')
            )
        else:
            self.x = torch.tensor(
                np.loadtxt(self.dir / 'x_teste.csv', delimiter=',')
            )
            self.y = torch.tensor(
                np.loadtxt(self.dir / 'y_teste.csv', delimiter=',')
            )
...
if __name__ == '__main__':
    ...
    treino_set, teste_set = Iris(
        ...
        Path('..', 'Dados'),
        treino=False,
        target_transform=lambda x: x.argmax(),
    )
    ...
    for nome, parametro in modelo.named_parameters():
        ...
        nome += '.txt'
        np.savetxt(
            dir_camada / nome, parametro.cpu().detach().numpy(), delimiter=','
        )
